...
参数：  
- -t 指定密钥类型。如果没有指定则默认生成用于SSH-2的RSA密钥。
- -C comment 提供一个新注释

# 调试
## ssh -vT git@github.com 查看错误原因  
发现Trying private key用的id\_rsa，没用我自己起名的，所以配置ssh\_config
```
...
# ssh_config
linux配置"/etc/ssh/ssh_config"文件
window配置"C:\Program Files\Git\etc\ssh\ssh_config"文件
名称 | 值 | 说明
---|---|---
